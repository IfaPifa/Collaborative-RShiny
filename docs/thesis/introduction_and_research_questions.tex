...
\begin{itemize}[nosep]
  \item \textbf{State field match rate} --- percentage of JSON fields (inputs + outputs) that are identical after restore. For deterministic apps (Calculator, Visual Analytics, Data Exchange), this should be 100\%. For stochastic apps (Monte Carlo, ML Trainer), exact reproducibility requires \texttt{set.seed()} --- without it, only the checkpoint mechanism preserves the exact result, because recomputation would produce different outputs.
